...
| `imgPos` (v1.4) | **dropped** — `background-position` stays `center` | USS has no `background-position`, so Figma's crop offset cannot be expressed; the size still comes through via `imgSize`, only the offset is lost. Recorded as known-loss per element |
