...
9. **Motion is solved once, mirrored byte-for-byte.** `figma2html/scripts/motion.py` is the master; `figma2godot` / `figma2unity` / `figma2unreal` / `figma2cocos` carry byte-identical copies (self-containment again — see principle 3), and `tools/conformance` fails on any drift, then independently checks that all four bake the *same sampled points*. Never map a Figma easing onto an engine's built-in enum: they share names and differ in shape, which is the silent-divergence trap this whole layer exists to close. If Figma doesn't publish control points for a named curve, leave it `unresolved` and let the caller log a known-loss — a curve that is 20 points off looks completely normal in the artifact and wrong on screen.
