...
> `flow_from_figma.py` 一直逐条报告而不猜,现在直接落地。没有任何字段改形状:它只是
> `@any:` / `@panelOutside:` 之外多一种特殊字符串写法,v1.0 的文件照样合法,
> v1.0 的消费者也只会在用到它的文件里遇到它。
...
- **events[]**:`on`(目前 click)+ `el`(figma node id;数组=多个触发同一动作;特殊 `@any:<modal>` / `@panelOutside:<modal>`,以及 v1.1 起的 `@in:<modal>:<nodeId>`,见下)+ 可选 `guard`(state 里这些都为真才放行)+ `do`(动作)+ `arg`。
- **`@in:<modal>:<nodeId>`**(v1.1):弹窗**内部**的元素。v1.1 之前 `events[].el` 只能指 **base 屏**的节点(各后端的 binder 都是拿 id 去 base 图层解析),于是真实 figma 文件里最常见的那条连线 —— **弹窗里的 ✗ 按钮** —— 根本没有表达;v1.0 只能拿 `@any:` / `@panelOutside:` 近似成「点哪都关 / 点面板外关」,那与「点这个按钮」不是一回事。解析规则:`@in:` 之后到**第一个**冒号为止是弹窗名,其余整段都是节点 id —— 因为 figma 的 id 自带冒号(`@in:bag:4:99`)。该节点必须落在这个弹窗**真正抬起来的那棵子树**(它的 `roots`)里,而不只是「在这一屏里」:落在 roots 之外的节点运行时根本不在那一层,离线校验器查的就是这一点。各 binder 都会在它上面停止冒泡,否则同一层上的 `@any:` / `@panelOutside:` 会跟着触发一次,按下 ✗ 等于顺手又「点了面板外」。
