# -*- coding: utf-8 -*-
"""录 README 的 GIF:Edge 无头逐帧截图 → Pillow 编码。

无头浏览器没法录屏,所以走**分帧确定性重放**:每一帧起一个新页面,用 ?step=N 告诉
driver 演到哪一步、并把真实动画对象暂停在指定毫秒,再截一张。
同一个脚本跑两次得到同一串图。

用法:
    python3 shoot_gif.py <示例目录> <out.gif> [driver 名]
    driver 名默认取示例目录的名字(login / main),对应本目录的 gif_driver_<名>.js 与下面的 FRAMES。

⚠️ **只有声明式动画录得到。** 无头这一路**一次 rAF 都不会触发**(实测:一个每帧自增的
计数器跑完整个 virtual-time 预算仍然是 0),所以逐帧 JS 循环画出来的东西在这里是**静止**的。
CSS 过渡与 WAAPI 关键帧可以被 pause() 钉在任意毫秒,录得到 —— 示例里的飞行轨迹因此是
"先把路径采成关键帧、再交给浏览器播",而不是每帧算一个位置。
"""
import io
import os
import shutil
...
# (step, 这一帧在 GIF 里停留多少毫秒)。step 的含义在对应的 gif_driver_*.js 的 STEPS 里 ——
...
FRAMES = {
    ...
    "main": [
        (0, 900),                        # 静止
        (1, 120), (2, 120), (3, 700),    # 背包下滑(figma 的 MOVE_IN)+ 格子逐项 → 停住
        (4, 140),                        # 背包出场中途(preset 补的 SCALE_OUT)
        (5, 700),                        # 图鉴(figma 的 DISSOLVE)+ 列表克隆
        (6, 220),                        # 点被锁的页签 → guard 拦下 + 抖动中途
        (7, 120), (8, 120), (9, 140),    # 领奖:爆开 → 停顿看清个数 → 抛物线吸入
        # 落袋这一帧要多给点预算:整条编排(飞 1.2s + 数字滚 0.6s)全走定时器,
        # 预算不够就拍在半路上,数字停在旧值 —— 看着像"没到账"。
        (10, 1200, 4600),                # 落袋:数字已到账 + 胶囊高亮闪
    ],
}
...
def main():
    ...
    key = sys.argv[3] if len(sys.argv) > 3 else os.path.basename(login)
    if key not in FRAMES:
        raise SystemExit("没有 %r 这份分镜(可选: %s)" % (key, ", ".join(sorted(FRAMES))))
    frames, driver = FRAMES[key], "gif_driver_%s.js" % key
    edge = find_edge()
    ...
    with io.open(gif_html, "w", encoding="utf-8", newline="") as f:
        f.write(page)
    shutil.copy(os.path.join(here, driver), os.path.join(login, "_gif.js"))

    tmp = tempfile.mkdtemp(prefix="figkit_gif_")
    imgs, durs = [], []
    try:
        for item in frames:
            step, dur = item[0], item[1]
            budget = item[2] if len(item) > 2 else 2500
            ...
            subprocess.run([edge, "--headless=new", "--disable-gpu", "--no-sandbox",
                            "--window-size=540,960", "--virtual-time-budget=%d" % budget,
