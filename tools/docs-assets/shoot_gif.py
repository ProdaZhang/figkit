...
FRAMES = {
    ...
    "mail": [
        (0, 1000),                       # 静止:邮件列表
        (1, 110), (2, 110), (3, 900),    # 详情入场(SCALE_IN)→ 停住
        (4, 140),                        # 领取:同一面板换态中途(DISSOLVE)
        (5, 900),                        # 停住:已领取态(水印 + 绿色删除)
        (6, 140),                        # 弹窗里的按钮关闭 · 出场中途(SCALE_OUT)
        # 这几步都要先点开详情再操作,整条链走的是定时器,预算不够会拍在半路上。
        (7, 1100, 4600),                 # 无附件的那封:只有删除,没有领取
    ],
}
