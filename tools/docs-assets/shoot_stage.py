...
用法:
    python3 shoot_stage.py <示例目录> <out.png>
...
拍完的图与引擎侧逐像素可比。实测(examples/main,对 Godot 4.3):平均差 **2.18/255**,
...
TOPBAR = 36                     # mountStage 里那句 `innerHeight - 36`
...
def main():
    if len(sys.argv) < 3:
        raise SystemExit("用法: python3 shoot_stage.py <示例目录> <out.png>")
    src = os.path.abspath(sys.argv[1])          # 相对路径会拼出不存在的 file:// URL
    out = sys.argv[2]
