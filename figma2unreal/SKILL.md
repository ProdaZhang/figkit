...
2. **转 uespec**:
   ...
   ```
   python scripts/ui_to_uespec.py <cap.ui.json> <outdir>                # 单屏
   python scripts/ui_to_uespec.py <cap.ui.json> <flow.json> <outdir>   # 整套:flow.caps 引用的所有屏
...
- **uespec 输出必须确定性**(UTF-8 无 BOM、`\n`、固定键序):`tests/golden/` 逐字节把关,
  有意变更格式必须重生成金样并说明。
- **改转换器必跑** `python scripts/tests/run_all.py`,全绿 exit 0 才算完。
