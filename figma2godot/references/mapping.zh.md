...
| `radius: "50%"`(capture 对**每个 ELLIPSE** 都产) | 正方形 → StyleBoxFlat 的 `corner_radius`;**非正方形的实色块 → 编译期落一份带椭圆角的 `.svg`**(`radius_axes` + `rounded_rect_d`),按纹理引入 | Godot 的 `corner_radius` 是**标量**,画不出椭圆角:2143×680 的椭圆被标量口径画成胶囊,顶弧**比 HTML 平 38px**,邮件面板下沿的黄色带在 y=1176 就断了而不是 y=1214。这条没留成 known-loss,而是改走 v1.2 矢量已经在走的那条路 —— 吐 `.svg`、由 ThorVG 栅格化:确定性、不下任何位图,对齐对象是 **CSS**(水平半径按宽、垂直按高,再按 §5.5 等比收缩),不是别家后端的将就实现。**仍会退回胶囊的**:同时带渐变/图片/描边/阴影/裁剪的那些 —— 它们还得靠 StyleBoxFlat 或纹理,逐元素留痕。此行曾因 `float('50%')` 抛异常而**整个丢掉**(椭圆渲染成方块且无告警),现由 tools/conformance 守着 |
